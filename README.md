...
### 2. 실행
```bash
# 기본 시스템 실행 (AI 추천 TOP 5 자동 분석)
uv run python main.py

# 인터랙티브 단일 종목 분석 (사용자 입력 방식)
uv run python report_single_ticker.py
...
[AI 추천 TOP 5 종목 자동 분석]
```

**인터랙티브 분석 모드 (report_single_ticker.py)**
```
🎯 단일 종목 투자 분석 시스템
분석하고 싶은 미국 주식의 티커를 입력하세요.
종료하려면 'q' 또는 'quit'을 입력하세요.

📝 티커 입력 (예: AAPL, TSLA, MSFT): MSFT
...
```
my-first-ai-agent/
├── main.py                           # 전자동 분석: AI 추천 TOP 5 종목 자동 분석
├── report_single_ticker.py           # 인터랙티브 분석: 사용자 입력 기반 단일 종목 분석
