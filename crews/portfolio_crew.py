...
class PortfolioCrew():
    ...
    def analyze_current_position(self) -> Task:
        return Task(
            description="Analyze the client's current position in {ticker} with average cost of ${avg_cost} USD. Read and analyze all available reports in the output folder including research reports, investment analysis, and translated reports. Compare current market price with client's average cost and assess the position's performance.",
            expected_output="A comprehensive analysis of the client's current position including unrealized P&L, position performance metrics, and comparison with current market conditions and investment recommendations from reports.",
    ...
    def assess_portfolio_risk(self) -> Task:
        return Task(
            description="Based on the position analysis and available investment reports, assess the risk profile of maintaining, increasing, or decreasing the position in {ticker}. Consider the client's current cost basis of ${avg_cost} and provide risk-adjusted scenarios.",
            expected_output="Risk assessment covering downside risk, upside potential, risk-reward ratios for different actions (buy more, hold, sell), and recommended position sizing based on current analysis.",
    ...
    def provide_portfolio_recommendation(self) -> Task:
        return Task(
            description="Based on the position analysis and risk assessment, provide clear recommendations for the client's {ticker} position with average cost ${avg_cost}. Consider all available analysis reports and provide specific action recommendations.",
            expected_output="Clear Korean recommendations including: 1) 추천 행동 (추가매수/보유/손절), 2) 구체적인 실행 계획, 3) 리스크 관리 방안, 4) 목표 수익률 및 손절선, 5) 포지션 사이즈 조절 제안. Format as actionable advice for Korean investors.",
